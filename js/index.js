...
function crearTarjeta(padreTarjeta, tarjeta) {
  ...
  nuevaTarjeta.innerHTML = `<div  class="card" style="width: 18rem;">
    ...
    <div class="card-body" >
      ...
      <div class="detalles">
        <p>$${tarjeta.price}</p>
        <a href="/details.html?value=${tarjeta._id}" id="${tarjeta._id}" class="btn btn-primary">Details</a>
      </div>
    </div>
  </div>`
...
function pintarChecks(arregloPinta, padre) {
  ...
  for (let i = 0; i < arregloPinta.length - 1; i++) {
    if (arregloPinta[i + 1].category != arregloPinta[i].category) {
      crearCheck(padre, arregloPinta[i])
    }

  }
  crearCheck(padre, arregloPinta[arregloPinta.length - 1]);
}
